...
def eval_epoch_with_thresholds(model, data, config, thresholds):
    ...
    if config["cuda"]:
        thresh = [t.cuda(device) for t in thresh]
...
def prettify_prediction_interpretation(interpretation_info, prediction_interpretation, config):
    ...
    * a table of all the prediction data:
    ...
    colors = ["#FFFF00", "#6698FF", "#E56717", "#00FF7F", "#FFA07A", "#FF8C00", "#846A20"
              "#755C48", "#DAD871", "#FAEBD7", "#34C924", "#212121", "#452D35", "#523C36"]  # add more colors here
    class_to_color = {}
    for cl, color in zip(list(config["class_to_str"]), colors*3):
